...
最近在看 CSAPP(Computer Systems: A Programmer's Perspective)这本书，虽然我看的是中文版，但是中文的书名翻译的确不太合适。这本书确实是书如其名，作为一个工作多年的程序员， 看起来如沐春风。印象最深刻的就是并发编程那章节介绍的一种可视化分析信号量的工具——进度图。本文就展开介绍一下它，在介绍进度图之前，我先啰嗦一下信号量，它提供了一种可以实现同步的原语，也是 Go 语言的关键组件之一。目前为止，我的每篇文章都离不开 Go😈。

## 信号量介绍

**信号量**（**semaphore**）是荷兰计算机科学家 Dijkstra 提出的（我最近成了他的迷弟），信号量命名应该是源于铁路的信号灯，用来控制进程的“走”还是“停”，避免忙等。
信号量是一个非负整数，除了初始化之外只有 PV 两个操作：
...
golang 的信号量实现在 sema.go，是基于之前 Google 为 Plan9 操作系统开发的”用户态“信号量。https://swtch.com/semaphore.pdf 。该设计基于硬件支持原子比较替换（`CAS`）和原子加法（`XADD`），后者是可以基于前者实现的。
...
Plan9 的信号量对标的是 Linux 的 futex，为了解决传统同步机制的一些问题，他们的优化方向是一样的，尽量保持在用户态，避免换到内核态，当然也吐槽了 futex 发布时给出的例子是错的，直到一年后才给出了正确的版本。用户态信号量比内核态信号量的好处是在非竞争时可以避免从用户态切换到内核态。

Plan9 还用信号量实现了自己的互斥锁代理 spin lock。 spin lock 虽然在内核态工作良好，在用户态可能失效，因为用户态可能会被操作系统的进程调度抢占。Plan9 最初的实现是，在尝试一千次失败后 用 sleep(0) 重新获取调度时间片，最终退化到 sleep(1ms)。用信号量代替 spinlock 的好处就是不受抢占影响，不过这显然不是最优的方案。

## Go 的信号量生态

下图是我整理的 Go 基于信号量的互斥生态，信号量的实现是基于内部的工业级互斥锁，提供给用户的很多组件都是基于信号量。可以清晰的看到，信号量是整个 Go 语言的关键组件。
...
251 是一个素数，对于每个信号量的地址，右移 3 位后对 251 取模哈希到不同的树中，这样是为了让内存中相邻的信号量尽量哈希到同一个树中，保持一种亲和性。
...
进度图（progress graph）可以将 n 个并发线程的执行顺序表达为一个 n 维的笛卡尔坐标系。每当我们想要分析并发问题的时候，第一个想到的工具大概是**时序图**，而进度图是非常冷门的，相关的资料非常少，当我在 CSAPP 书中发现它时，就好像捡到一本失传已久的武林秘籍。在某些场景下，进度图提是比时序图更直观的且精准的并发分析方法。为了简单直观，我们可以用二维笛卡尔坐标系表达两个并发逻辑模型，下面介绍一下进度图的基本概念和分析方法。
...
多元信号量的不安全区不太好直观的展现出来，这里我引入一个新的组件**栏杆**来表示可能存在的一种情况，即**挡住来路**。另外，由于多元信号量的初始值可能有多种情况，所以我需要把情况拆开来分析。
...
可以看到，多元信号量比二元信号量需要更多的初始情况，这就需要画更多的进度图来分析不同的情况，这虽然增加了很多工作，但进度依然是可靠的。然而这还不是最复杂的情况，如果 PV 操作是有前提条件的话，进度图就更加捉襟见肘了，比如用信号量实现的**读写锁**
...
这里我发现一个有趣的事情，对于同一坐标点，不同的行进路线会产生不同的值，这代表一个坐标点存在多个值，只不过对于同一条行进路线来说值是不变的，这就增加了更多的分析路径，但依然可以通过进度图来分析它的安全性。
